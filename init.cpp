...
int main()
{
    ...
    for (int i = 0; i < chain.size(); i++)
    {
        ...
        out << "const complex<double> omega" << i << " = complex<double>(0.0001, 0.0);" << "\n";
        ...
        if (i == 0)
        {
            ...
            equations << "dxdt[" << i << "] = (-I) * ((nu" << i << " + x" << i << " * omega" << i << " * x[" << chain.size() + 2 * i << "]) * x[" << i << "] + nu" << i << i + 1 << " * x[" << i + 1 << "]);" << "\n";
        }
        ...
        else
        {
            ...
            out << "const complex<double> nu" << i << " = complex<double>(" << (chain[i] == 'G' ? "124.0/6.582119514, 0.0);" : (chain[i] == 'A' ? "169.0/6.582119514, 0.0);" : "190.0/6.582119514, 0.0);")) << "\n";
            out << "const complex<double> nu" << i << i + 1 << " = complex<double>(1.92, 0.0);" << "\n\n";
            equations << "dxdt[" << i << "] = (-I) * ((nu" << i << " + x" << i << " * omega" << i << " * x[" << chain.size() + 2 * i << "]) * x[" << i << "] + nu" << i << i - 1 << " * x[" << i - 1 << "]" << " + nu" << i << i + 1 << " * x[" << i + 1 << "]);" << "\n";
        }
    }
    ...
    for (int i = chain.size(), k = 0; i < 3 * chain.size(); i += 2, k++)
    {
        ...
        equations << "dxdt[" << i + 1 << "] = -omega_" << k << "_shtrix * x[" << i + 1 << "] - omega" << k << " * x[" << i << "] - norm(x[" << k << "]);" << "\n";
    }
